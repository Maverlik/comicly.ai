...
The Vercel entrypoint is `backend/api/index.py`, which imports the FastAPI app from `backend/app/main.py`; `backend/vercel.json` rewrites incoming paths to that function. Configure the Vercel function max duration to 300 seconds in project settings where supported.
